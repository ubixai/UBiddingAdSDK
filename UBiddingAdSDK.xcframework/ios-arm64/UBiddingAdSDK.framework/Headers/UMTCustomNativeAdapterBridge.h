//
//  UMTCustomNativeAdapterBridge.h
//  UMTAdSDK
//
...
#ifndef UMTCustomNativeAdapterBridge_h
#define UMTCustomNativeAdapterBridge_h

#import "UMTCustomNativeAdapter.h"
#import "UMTMediationNativeAd.h"
#import "UMTGlobalDefine.h"

@protocol UMTCustomNativeAdapter;

/// 自定义Native广告的adapter的回调协议
@protocol UMTCustomNativeAdapterBridge <NSObject>
@optional
...
/// @param ads 广告数据组
/// @param exts 扩展数据组
- (void)nativeAd:(id<UMTCustomNativeAdapter>_Nonnull)adapter didLoadWithNativeAds:(NSArray<UMTMediationNativeAd *> * _Nullable)ads exts:(NSArray<NSDictionary *> * _Nullable)exts;

/// 在模板广告加载完成时调用该方法，直接调用即可，无需做响应判断
/// @param adapter 当前适配器
/// @param views 广告视图数据组
/// @param exts 扩展数据组
- (void)nativeAd:(id<UMTCustomNativeAdapter>_Nonnull)adapter didLoadWithExpressViews:(NSArray<UIView *> * _Nullable)views exts:(NSArray<NSDictionary *> * _Nullable)exts;

/// 在广告加载失败时调用该方法，直接调用即可，无需做响应判断
/// @param adapter 当前适配器
/// @param error 错误信息
- (void)nativeAd:(id<UMTCustomNativeAdapter>_Nonnull)adapter didLoadFailWithError:(NSError *_Nullable)error;
...
- (void)nativeAd:(id<UMTCustomNativeAdapter>_Nonnull)adapter renderSuccessWithExpressView:(UIView *_Nonnull)expressView;

/// 仅限模板广告，在渲染失败调用，直接调用即可，无需做响应判断
/// @param adapter 当前适配器
/// @param expressView 模板广告
- (void)nativeAd:(id<UMTCustomNativeAdapter>_Nonnull)adapter renderFailWithExpressView:(UIView *_Nonnull)expressView andError:(NSError *_Nonnull)error;

/// 仅限模板广告，在模板广告关闭的时候调用，直接调用即可，无需做响应判断
/// @param adapter 当前适配器
/// @param expressView 模板广告
/// @param filterWords 用户手动关闭时的关闭原因描述
- (void)nativeAd:(id<UMTCustomNativeAdapter>_Nonnull)adapter didCloseWithExpressView:(UIView *_Nonnull)expressView closeReasons:(NSArray<NSString *> *_Nullable)filterWords;

#pragma mark - non-Express ad
/// 仅限非模板广告，在非模板广告点击不喜欢原因后调用
/// @param adapter 当前适配器
/// @param nativeAd 广告数据，native广告请传递原始数据（即UMTMediationNativeAd.originMediatedNativeAd）
/// @param filterWords 用户手动关闭时的关闭原因描述
...
#endif /* UMTCustomNativeAdapterBridge_h */
